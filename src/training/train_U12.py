...
X_add_distance_train = np.concatenate((
                          data_x_5_dist.to_numpy().reshape((-1, 70, 1))[np.logical_and(train_index, u12_index)],
                          data_x_3_dist.to_numpy().reshape((-1, 70, 1))[np.logical_and(train_index, u12_index)]),
                          axis = -1)

X_add_distance_valid = np.concatenate((
                          data_x_5_dist.to_numpy().reshape((-1, 70, 1))[np.logical_and(valid_index, u12_index)],
                          data_x_3_dist.to_numpy().reshape((-1, 70, 1))[np.logical_and(valid_index, u12_index)]),
                          axis = -1)

X_train_oh = data_x_oh[np.logical_and(train_index, u12_index)]
X_train_kmers = data_x_kmers_embedding[np.logical_and(train_index, u12_index)]
X_train_word2vec = data_x_word2vec_embedding[np.logical_and(train_index, u12_index)]

X_valid_oh = data_x_oh[np.logical_and(valid_index, u12_index)]
X_valid_kmers = data_x_kmers_embedding[np.logical_and(valid_index, u12_index)]
X_valid_word2vec = data_x_word2vec_embedding[np.logical_and(valid_index, u12_index)]

y_train = tf.reshape(data_y_target, (-1, 70, 1))[np.logical_and(train_index, u12_index)]
y_valid = tf.reshape(data_y_target, (-1, 70, 1))[np.logical_and(valid_index, u12_index)]
##

## model transfer learning
oh_model = tf.keras.models.load_model(model_save_path + '/all_U2_oh.h5', 
...
sub_model.ModelTrainer(oh_model).train((X_train_oh, X_add_distance_train), (X_valid_oh, X_add_distance_valid),
                                        y_train, y_valid, 
                                        PATIENCE = 15, EPOCHS = 1000, BATCH_SIZE = 5, VERBOSE = 1)

kmers_model = tf.keras.models.load_model(model_save_path + '/all_U2_kmers.h5', 
                                         custom_objects = {'SelfAttention':sub_model.SelfAttention})
...
word2vec_model = tf.keras.models.load_model(model_save_path + '/all_U2_word2vec.h5', 
                                            custom_objects = {'SelfAttention':sub_model.SelfAttention})
...
oh_model.save(model_save_path + '/all_U12_oh.h5')
kmers_model.save(model_save_path + '/all_U12_kmers.h5')
word2vec_model.save(model_save_path + '/all_U12_word2vec.h5')